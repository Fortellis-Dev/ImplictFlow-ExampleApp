This project was bootstrapped with [Create React App](https://github.com/facebookincubator/create-react-app), whose readme is preserved below.

This is an example of a (very) simple single-page react app used to demonstrate how to acquire [Fortellis](https://fortellis.io) access tokens using the Implicit Grant flow. It is accompanied by a [user guide](https://community.fortellis.io/community/blog/acquiring-oauth-tokens-implicit-flow) that will guide users through the entire process of creating and deploying an app than can call Fortellis APIs.

The app performs the implicit grant flow to acquire a token and then uses that token to call the [Merchandisable Vehicles API](https://developer.fortellis.io/api-reference/vehicle-sales/merchandisable-vehicles-api) to get the vehicles available to buy at a test dealership. A few key details of these vehicles are displayed to the user.

Interested parties can play with a deployed version of the app [here](http://fortellis-vehicle-finder.s3-website.us-east-2.amazonaws.com/).
